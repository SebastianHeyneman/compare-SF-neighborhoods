...
with open(fileName) as csvfile:
    ...
    for row in reader:
        ...
        if coor == ' ,   ' or coor == ', ':
            coorDict[row['IncidntNum']] = 'No Data' #PrimaryKey is my primary key that I will use to write the data back into the .csv
        else:
            if float(row['longitude']) > 0:
                coorDict[row['IncidntNum']] = (rlat,rlong)
            else:
                coorDict[row['IncidntNum']] = (rlong,rlat)
        m += 1

rows = [["IncidntNum", "nhood"]]
...
with open("incidentsMatchedNeighborhood.csv", "w") as f:
    writer = csv.writer(f)
